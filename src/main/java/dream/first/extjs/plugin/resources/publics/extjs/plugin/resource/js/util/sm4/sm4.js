 ...
 * @returns 加密后的数据
 */
function sm4Encode(input,sm4Key){
	return SM4.encode({key:sm4Key,input:input});
}
/**
 * 通过sm4解密数据
 * @param input 加密的数据
 * @returns 解密后的数据
 */
function sm4Decode(input,sm4Key){
	return SM4.decode({key:sm4Key,input:input});
}
