...
Ext.define('Ext.ux.window.Notification', {
	...
	initComponent: function() {
		...
		Ext.apply(me, {
			bbar: [{
			    ...
			    inputValue : "1",
			    listeners : {
					change : me.notifyListeners["noNotifyChecked"] || Ext.emptyFn
				}
			},"->",{
				xtype : "button",
				text : "查看",
				handler : me.notifyListeners["viewNotifyContent"] || Ext.emptyFn
			}]
		});
	...
	destroy: function () {
		...
		//调用窗口destroy监听器
		if (me.notifyListeners) {
			if (me.notifyListeners["afterdestroy"]) {
				me.notifyListeners["afterdestroy"]();
			}
		}
	}
